...
<html>
	...
	<body>
		<div class="nosac_sa_tabelom">
			...
			$broj_kalk=$red1['dos_kal'];

			$upit2 = mysql_query("SELECT sif_firme FROM kalk
			WHERE broj_kalk='$broj_kalk'");
			$red2 = mysql_fetch_array($upit2);
			$sif_firme=$red2['sif_firme'];
				...
				<tr>
					...
					<td class="velikaslova">
						...
						$upit=mysql_query("SELECT 
											SUM(((((ulaz.cena_k/100)*(100-k_pism_tr.rabat_p))/100)*(100+roba.porez))*k_pism_tr.kolic_p)AS teret,
											SUM(k_pism_tr.kolic_p*ulaz.cena_k) AS kol_i_cena_s,
											SUM(((ulaz.cena_k/100)*(100-k_pism_tr.rabat_p))*k_pism_tr.kolic_p) AS bez_pdv_teret, 
											SUM((ulaz.cena_k-((ulaz.cena_k/100)*(100-k_pism_tr.rabat_p)))*k_pism_tr.kolic_p) AS k_rab
											FROM k_pism_tr 
											RIGHT JOIN ulaz ON k_pism_tr.sif_rob_p=ulaz.srob_kal AND k_pism_tr.id_u_i=ulaz.id
						...
						while ($niz=mysql_fetch_array($upit))
						{
							...
							$teret= $niz['teret'];
							$bez_pdv_teret=number_format($niz['bez_pdv_teret'], 2,".",",");
							$pdv=$niz['teret']-$niz['bez_pdv_teret'];
							$k_rab= $niz['k_rab'];
			...
			<table>
				...
				<?php 
				$upit4=mysql_query("SELECT k_pism_tr.id_k, k_pism_tr.sif_rob_p, k_pism_tr.kolic_p, k_pism_tr.rabat_p, ulaz.cena_k, roba.naziv_robe, 
								(k_pism_tr.kolic_p*ulaz.cena_k) AS kol_i_cena 
								FROM k_pism_tr 
								RIGHT JOIN ulaz ON k_pism_tr.sif_rob_p=ulaz.srob_kal AND k_pism_tr.id_u_i=ulaz.id 
								LEFT JOIN roba ON k_pism_tr.sif_rob_p=roba.sifra 
								WHERE broj_p='$br_k_pis'");
				...
				{?>
					<tr>
						...
						<td><?php echo $niz2['cena_k'];?></td>
						<td><?php echo $niz2['kol_i_cena'];?></td>
						<td><?php echo $niz2['rabat_p'];?></td>
						<td class=print_hide>
							<form action='k_pis_r_k4bris.php' method='post'>
			...
			<form action="k_pis_r_k5.php" method="post">
				<input type="hidden" name="br_k_pis" value="<?php echo $br_k_pis; ?>"/>
